defmodule Helios.Aggregate.Pipeline.Builder do
  @type plug :: module | atom

  @moduledoc """
  Builder for Aggregate pipeline
  """
  ...
  defmacro __using__(opts) do
    quote do
      ...
      import Helios.Aggregate.Pipeline.Builder, only: [plug: 1, plug: 2]

      Module.register_attribute(__MODULE__, :plugs, accumulate: true)
      @before_compile Helios.Aggregate.Pipeline.Builder
    end
  end
  ...
  defmacro __before_compile__(env) do
    ...
    {ctx, body} = Helios.Aggregate.Pipeline.Builder.compile(env, plugs, builder_opts)
  ...
  @doc false
  @spec compile(Macro.Env.t, [{plug, Helios.Aggregate.Pipeline.Plug.opts, Macro.t}], Keyword.t) :: {Macro.t, Macro.t}
  ...
  defp quote_plug({plug_type, plug, opts, guards}, acc, env, builder_opts) do
    ...
    error_message = case plug_type do
      :module   -> "expected #{inspect plug}.call/2 to return a Helios.Aggregate.Pipeline.Context"
      :function -> "expected #{plug}/2 to return a Helios.Aggregate.Pipeline.Context"
    end <> ", all plugs must receive context and return context"

    quote do
      case unquote(compile_guards(call, guards)) do
        %Helios.Aggregate.Pipeline.Context{halted: true} = data ->
          unquote(log_halt(plug_type, plug, env, builder_opts))
          data
        %Helios.Aggregate.Pipeline.Context{} = data ->
          unquote(acc)
        _ ->
